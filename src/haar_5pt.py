...
# -------------------------
# Multi-face Haar + FaceMesh(ROI) 5pt (Backported from recognize.py)
# -------------------------
class HaarFaceMesh5pt:
    ...
    def detect(self, frame_bgr: np.ndarray, max_faces: int = 5) -> List[FaceKpsBox]:
        H, W = frame_bgr.shape[:2]
        gray = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)
        faces = self._haar_faces(gray)
        if faces.shape[0] == 0:
        ...
        for x, y, w, h in faces:
            ...
            # Clip ROI
            rx1 = int(max(0, min(W - 1, round(x - mx))))
            ry1 = int(max(0, min(H - 1, round(y - my))))
            rx2 = int(max(0, min(W - 1, round(x + w + mx))))
            ry2 = int(max(0, min(H - 1, round(y + h + my))))
